...
public sealed class TemplateBasedDocumentExportService(StorageOptions options) : IDocumentExportService
{
    ...
    private static void GenerateFromTemplate(GeneratedDocument document, string outputPath)
    {
        ...
        RemoveExternalRelationships(outputPath);
    }
    ...
    private static void SetCoreProperties(WordprocessingDocument document, GeneratedDocument generated)
    {
        ...
        var title = $"LiCvWriter generated {generated.Kind} document";
        const string description = "Generated by LiCvWriter.";
    ...
    private static void RemoveExternalRelationships(string outputPath)
    {
        using var archive = ZipFile.Open(outputPath, ZipArchiveMode.Update);
        XNamespace relationshipsNamespace = "http://schemas.openxmlformats.org/package/2006/relationships";

        foreach (var entry in archive.Entries
            .Where(static entry => entry.FullName.EndsWith(".rels", StringComparison.OrdinalIgnoreCase))
            .ToArray())
        {
            ...
            var externalRelationships = relationships.Root?
            ...
            entry.Delete();
            var replacementEntry = archive.CreateEntry(fullName, CompressionLevel.Optimal);
            using var replacementStream = replacementEntry.Open();
            relationships.Save(replacementStream, SaveOptions.DisableFormatting);
        }
    }
